...
;       /square: if set, make the plots square
; Outputs:
...
;       
;       Added /square keyword
;       Work in device coordinates so we can force aspect ratio to
;       be square if requested.
;            Erin Scott Sheldon 20-Mar-2001 UofMich
;
;-
...
;on_error,2
  ...
  IF keyword_set(square) THEN sqplot=1
;
  ...
  if init or keyword_set(initialize) then begin
      ...
      margins = [min(!x.window)-min(!x.region), $ ; in normlized coordinates
                 ...
                 max(!y.region)-max(!y.window)]
                                ...
                                ;in device coord
      margins[0] = margins[0]*!d.x_vsize
      margins[2] = margins[2]*!d.x_vsize
      margins[1] = margins[1]*!d.y_vsize
      margins[3] = margins[3]*!d.y_vsize
...
;
  ...
  idx = [(!d.x_vsize-margins[0]-margins[2])/cols, $
         (!d.y_vsize-margins[1]-margins[3])/rows] ; device coords per plot

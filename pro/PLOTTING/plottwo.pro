...
PRO plottwo, x1, y1, x2, y2, _extra=extra, center=center, $
...
;  IF (np LT 2) OR (np GT 3) THEN BEGIN 
  IF np LT 4 THEN BEGIN 
     print,'-Syntax: plottwo, x1, y1, x2, y2, _extra=extra, center=center, $'
     ...
     print,'         xrange=xrange, topyrange=topyrange, botyrange=botyrange,$'
     print,'         topylog=topylog,$'
     print,'         botylog=botylog,$'
     print,'         botxminor=botxminor,botyminor=botyminor,$'
     print,'         topxminor=topxminor,topyminor=topyminor,$'
     print,'         toppsym=toppsym, botpsym=botpsym,$'
     print,'         xoplot=xoplot, $'
     print,'         yoplot=yoplot, $'
     print,'         oploterr=oploterr,$'
     print,'         oplotcolor=oplotcolor,$'
     print,'         oplotsym=oplotsym, $'
     print,'         xticklen=xticklen,$'
     ...
     print,'         xtickformat=xtickformat, $'
     print,'         ytickformat1=ytickformat1, $'
     print,'         ytickformat2=ytickformat2, $'
     print,'         title=title'
     print,''
  ...
  plot, x1, y1, position=position1, /device, /noerase, _extra=extra, $
        ...
        ylog=topylog, psym=toppsym,linestyle=toplinestyle, $
  ...
  IF (n_elements(xoplot) NE 0) AND (n_elements(yoplot) NE 0) THEN BEGIN 
      IF n_elements(oploterr) NE 0 THEN BEGIN 
          oploterror, xoplot, yoplot, oploterr, psym=oplotsym, $
                      linestyle=oplotlinestyle, $
                      color=oplotcolor, errc=oplotcolor
      ENDIF ELSE BEGIN
          oplot, xoplot, yoplot, psym=oplotsym, line=oplotlinestyle, $
                 color=oplotcolor
      ENDELSE 
  ENDIF 
  ...
  plot, x2, y2, position=position2, /device, /noerase, _extra=extra, $
        ...
        xtickf=xtickformat, ytickf=ytickformat2

...
;                      repeat begin
;                          thisHDU = mrdfits3(lun, 0, hdr, status=status)
;                          ... process the HDU ...
...
;            names were changed to mrdfits3*
...
;=============================================================================
function mrd3_substruct, table, column
;
...
function mrd3_getc, table, column
    ...
    ; in a structure.
    if not mrd3_substruct(table, column) then begin
       return, table.(column)  
    endif else begin
...
function mrd3_sizcol, table, column
...
pro mrd3_putc, table, column, value
...
function  mrd3_dofn, name, index, use_colnum, alias=alias
...
function mrd3_chkfn, name, namelist, index 
...
function mrd3_unsigned_offset, type
...
function mrd3_chkunsigned, bitpix, scale, zero, unsigned=unsigned
...
function mrd3_unsignedtype, data
...
; Return the currrent version string for MRDFITS3
function mrd3_version
    return, '2.6'
end
;=====================================================================
...
pro mrd3_read_ascii, unit, range, nbytes, nrows, nfld, typarr, posarr,   $
	    ...
	    endelse
            ...
            if typarr[i] eq 'I' then begin
                if (not keyword_set(old_struct)) then begin
                    ...
                    mrd3_putc, table, i, long(flds)
                endelse
            endif else if typarr[i] eq 'E' or typarr[i] eq 'F' then begin
                if (not keyword_set(old_struct)) then begin
                    table.(i) = float(flds)
                endif else begin
                    mrd3_putc, table, i, float(flds)
                endelse
            endif else if typarr[i] eq 'D' then begin
                if (not keyword_set(old_struct)) then begin
                    table.(i) = double(flds)
                endif else begin
                    mrd3_putc, table, i, double(flds)
                endelse
            endif else if typarr[i] eq 'A' then begin
                if (not keyword_set(old_struct)) then begin
                    table.(i) = flds
                endif else begin
                    mrd3_putc, table, i, flds
                endelse
            endif
        endelse
    endfor
...
pro mrd3_ascii, header, structyp, use_colnum,   $
    range, table, $
    ...
    if strmid(fxpar(header, 'XTENSION'),0,8) ne 'TABLE   ' then begin
        print, 'MRDFITS3: Header is not from ASCII table.'
        status = -1;
        return
    endif
    ...
    if nrows le 0 then begin
        if not keyword_set(silent) then begin
            print,'MRDFITS3: ASCII table.  ',strcompress(string(nfld)),  $
                  ' columns, no rows'
        endif
        return
    endif
	...
	if cnt eq 0 then fnull = ''
        ...
        fname = mrd3_dofn(fname,i+1, use_colnum, alias=alias)
        fnames[i] = fname

        fname = mrd3_chkfn(fname, fnames, i)
        
        mrd3_atype, fform, ftype, flen
        typarr[i] = ftype
        lenarr[i] = flen
        posarr[i] = fpos
        nullarr[i] = fnull
        ...
        print, 'MRDFITS3: Invalid format code:',ftype, ' for column ', i+1
        status = -1
        return
  next_col:
    endfor
...
; user specification.
pro  mrd3_columns, table, columns, fnames, fvalues, $
    ...
    if n_elements(tcols) le 0 then begin
        print, 'MRDFITS3:  No columns match'
        ...
        for i=0, n_elements(tcols)-1 do begin
            if  not keyword_set(old_struct) then begin
                ...
                mrd3_putc, tabx, i, mrd3_getc(table,tcols[i])
            endelse
        endfor
...
pro mrd3_read_image, unit, range, maxd, rsize, table 
    ...
    type = mrd3_unsignedtype(table)
    if type gt 0 then begin
	table = table - mrd3_unsigned_offset(type)
    endif
...
pro mrd3_axes_trunc,naxis, dims, silent
    ...
    for i=naxis-1,1,-1 do begin 
        ...
        if dims[i] eq 1 then begin
            if not mysilent then begin
                print, 'MRDFITS3: Truncating unused dimensions'
                mysilent = 1
            endif
            dims = dims[0:i-1] 
            naxis = naxis - 1 
...
pro mrd3_image, header, range, maxd, rsize, table, scales, offsets, scaling, $
  status, silent=silent, tempdir=tempdir, unsigned=unsigned
    ...
    xunsigned = mrd3_chkunsigned(bitpix,  xscale, $
	...
	print,'MRDFITS3: Error: Invalid BITPIX: '+strtrim(bitpix)
	table = 0
	return
    endif
	...
	maxd = gcount
        ...
        if (n_elements(dims) gt 1) then begin
            ...
            print, 'MRDFITS3: Warning: No data specified for group data.'
            dims = [0]
            naxis = 0
        endelse
        ...
        mrd3_axes_trunc, naxis, dims, keyword_set(silent)
        ...
        if not keyword_set(silent) then print,'MRDFITS3--Image with groups:', $
	...
	if xunsigned then begin
	    fxaddpar,header, 'BZERO', 0, 'Reset by MRDFITS3 v'+mrd3_version()
	endif
        ...
        if naxis eq 0 then begin
            ...
            table = 0
            if not keyword_set(silent) then begin
                print, 'MRDFITS3: Null image, NAXIS=0'
            endif
            return
        ...
        if not keyword_set(silent) then begin
            ...
            str = str+')'
            print, 'MRDFITS3: Image array ',str, '  Type=', typstrs[type]
        endif
	...
	if xunsigned then begin
	    fxaddpar,header, 'BZERO', 0, 'Updated by MRDFITS3 v'+mrd3_version()
	endif
...
pro mrd3_ptrscale, array, scale, offset
...
pro mrd3_scale, type, scales, offsets, table, header,  $
	...
	; Just copy the unscaled columns
        if ww[0] ne -1 then begin
            ...
            for i=0, n_elements(ww)-1 do begin
                if (not keyword_set(old_struct)) then begin
                    ...
                    mrd3_putc, tabx, ww[i], mrd3_getc(table,ww[i])
                endelse
            endfor
        endif
		...
		sz = size(tabx.(w[i]))
		if sz[sz[0]+1] eq 10 then begin
		    mrd3_ptrscale, table.(w[i]), scales[w[i]], offsets[w[i]]
		endif
                tabx.(w[i]) = table.(w[i])*scales[w[i]] + offsets[w[i]]
		
            endif else begin
		
		sz = mrd3_sizcol(table, w[i])
		if sz[sz[0]+1] eq 10 then begin
		    arr = mrd3_getc(table,w[i])
		    mrd3_ptrscale, arr, scales[w[i]], offsets[w[i]]
		    mrd3_putc, tabx, w[i], arr
		endif else begin
                    mrd3_putc, tabx, w[i], mrd3_getc(table,w[i])*scales[w[i]] + offsets[w[i]]
		endelse
		
            endelse
            istr = strcompress(string(w[i]+1), /remo)
            fxaddpar, header, 'TSCAL'+istr, 1.0, 'Set by MRD3_SCALE'
            fxaddpar, header, 'TZERO'+istr, 0.0, 'Set by MRD3_SCALE'
        endfor
    ...
    ; Now process images and random groups.
        ...
        if sz[sz[0]+1] ne 8 then begin
            ...
            endelse
            fxaddpar, header, 'BSCALE', 1.0, 'Set by MRD3_SCALE'
            fxaddpar, header, 'BZERO', 0.0, 'Set by MRD3_SCALE'
            ...
            for i=0, nparam-1 do begin
                istr = strcompress(string(i+1),/remo)
                fxaddpar, header, 'PSCAL'+istr, 1.0, 'Added by MRD3_SCALE'
                fxaddpar, header, 'PZERO'+istr, 0.0, 'Added by MRD3_SCALE'
                tabx.(0)[i] = table.(0)[i]*scales[i]+offsets[i]
            endfor
	    
            tabx.(1) = table.(1)*scales[nparam] + offsets[nparam]
            fxaddpar, header, 'BSCALE', 1.0, 'Added by MRD3_SCALE'
            fxaddpar, header, 'BZERO', 0.0, 'Added by MRD3_SCALE'
            table = temporary(tabx)
        endelse
    endelse
...
pro mrd3_varcolumn, vtype, array, heap, off, siz
    ...
    unsigned = mrd3_unsigned_offset(unsigned)
...
pro mrd3_fixcolumn, vtype, array, heap, off, siz
...
; length arrays. 
pro mrd3_read_heap, unit, header, range, fnames, fvalues, vcls, vtpes, table, $ 
   ...
   old_struct=old_struct, pointer_var=pointer_var, fixed_var=fixed_var
    ...
    if heapoff ne 0 and heapoff lt sz then begin 
        print, 'MRDFITS3: ERROR Heap begins within data area' 
        status = -1 
        return 
    endif
    ...
    if delete then begin
        ...
        if ww[0] ne -1 then  begin
            w[vcols[ww]] = 0
            if not keyword_set(silent) then begin
                print, 'MRDFITS3: ', strcompress(string(n_elements(ww))),  $
        ...
        if (wx[0] eq -1) then begin
            if not keyword_set(silent) then begin
                print, 'MRDFITS3: All columns have been deleted'
            endif
	    table = 0
	    return
        endif
    ...
    if not keyword_set(pointer_var) then begin
        ...
        if n_elements(lencols) gt 0 then begin
            if not keyword_set(silent) then begin
                print, 'MRDFITS3: ', strcompress(string(n_elements(lencols))), $
    ...
    for i=0, n_elements(fnames)-1 do begin
        ...
        if col ge 0 then begin
            ...
            if w[0] eq -1 then begin
                ...
                if not keyword_set(old_struct) then begin
                    ...
                    mrd3_putc, tablex, i, mrd3_getc(table,col)
                endelse
		...
		if varying and keyword_set(pointer_var) and vtype ne 'A' then begin
		    mrd3_varcolumn, vtype, curr_colx, heap, off, siz
		endif else begin
		    mrd3_fixcolumn, vtype, curr_colx, heap, off, siz
		endelse
            ...
            if not keyword_set(old_struct) then begin
                ...
                xx = mrd3_getc(table, ncol)
                mrd3_putc, tablex, i, reform(xx[0,*])
            endelse
        endelse
    endfor 
...
pro mrd3_read_table, unit, range, rsize, structyp, nrows, nfld, typarr, table, $
                                      old_struct=old_struct
  ...
  nxtfld: 
    ...
    for i=0, nfld-1 do begin
	    ...
	    type = mrd3_unsignedtype(table.(i))

	    if type gt 0 then begin
	        table.(i) = table.(i) - mrd3_unsigned_offset(type)
	    endif
	    
	endif else begin
	    
	    col = mrd3_getc(table,i)
	    type  = mrd3_unsignedtype(col)
	    
	    if type gt 0 then begin
		mrd3_putc, table, i, col - mrd3_unsigned_offset(type)
	    endif
...
pro mrd3_tdim, header, index, flen, arrstr, no_tdim=no_tdim
    ...
    if strmid(tdstr,0,1) ne '(' and strmid(tdstr,len-1,1) ne ')' or len lt 3 then begin
        print, 'MRDFITS3: Error: invalid TDIM for column', index
        return
    endif
    ...
    if prod ne flen then begin
        print,'MRDFITS3: Error: TDIM/TFORM dimension mismatch'
        return
    endif
...
pro mrd3_table, header, structyp, use_colnum,           $ 
    ...
    if xten ne 'BINTABLE' and xten ne 'A3DTABLE' then begin 
        print, 'MRDFITS3: ERROR - Header is not from binary table.' 
        nfld = 0 & status = -1 
        return 
    endif 
    ...
    for i=0, nfld-1 do begin
        ...
        if dim eq 1 then begin
            ...
            mrd3_tdim, header, i+1, dim, str, no_tdim=no_tdim
            dimfld[i] = str
        endelse
        ...
        if ftype eq 'P' then begin 
            ...
            if dim ne 0  and dim ne 1 then begin 
                print, 'MRDFITS3: Invalid dimension for variable array column '+string(i+1) 
                status = -1 
                return 
            endif
            ...
            if strpos('LXBIJKAEDCM', vf) eq -1 then begin 
                print, 'MRDFITS3: Invalid type for variable array column '+string(i+1) 
                status = -1 
                return 
            endif 
	    ...
	    xunsigned = mrd3_chkunsigned(bitpix[ppos], xscale,       $
		...
		xunsigned = mrd3_chkunsigned(bitpix[j], xscale, $
					   fxpar(header, 'TZERO'+istr), $
					   unsigned=unsigned)

		if xunsigned then begin
		    fxaddpar, header, 'TZERO'+istr, 0, 'Modified by MRDFITS3 V'+mrd3_version()
		endif
  ...
  next_col: 
    ...
    if N_zero GT 0 then begin
        ...
        if N_zero Eq nfld then begin
            print,'MRDFITS3: Error - All fields have zero length'
            return
        endif
	
        for i=0, N_zero-1 do begin
	    print,'MRDFITS3: Table column ' + fnames[zero[i]] + ' has zero length'
	endfor
    ...
    if not keyword_set(silent) then begin
        print, 'MRDFITS3: Binary table. ',strcompress(string(nfld)), ' columns by ',  $
          strcompress(string(nrow)), ' rows.'
        if n_elements(vcls) gt 0 then begin
                print, 'MRDFITS3: Uses variable length arrays'
        endif
    endif
...
function mrdfits3_new, file, extension, header,               $
                  deja_vu=deja_vu,                        $
                  structyp = structyp,                    $
  ...
  common mrdfits3_block, rsize, tab, nfld, typarr,        $ 
    fnames, fvalues, vcls, vtpes, scales, offsets, scaling, $
    nrows

    ;   Let user know version if MRDFITS3 being used.
    if keyword_set(version) then begin
        print,'MRDFITS3: Version '+mrd3_version()+' February 18, 2002'
    endif
	...
	if keyword_set(version) then return, 0
        ...
        print, '   a=mrdfits3(file/unit, [extension, header], /deja_vu, /version       $'
    ...
    if keyword_set(structyp) then begin
        ...
        if sz[1] ne 7 then begin
            print, 'MRDFITS3: stucture type must be a string'
            return, 0
        endif
    endif
    ...
    sz = size(file)
    if (sz[0] ne 0) then begin
        print, 'MRDFITS3: Vector input not supported'
        return, 0
    endif
    ...
    if sz[1] gt 0 and sz[1] lt 4 then begin
        ...
        if unit lt 0 then begin
            print, 'MRDFITS3: File access error'
            status = -1
            return, 0
        endif
    endelse
    ...
    if eof(unit) then begin
        print,'MRDFITS3: Extension past EOF'
    ...
    if status lt 0 then begin
        print, 'MRDFITS3: Unable to read header for extension'
	if inputUnit eq 0 then begin
            free_lun, unit
	endif
        return, 0
	...
	type = 2
        ...
        print, 'MRDFITS3: Unable to process extension type:', xten
	if inputUnit eq 0 then begin
	    free_lun,unit
	endif
	status = -1
    ...
    if type eq 0 then begin
        ...
        mrd3_image,header,arange,maxd,rsize,table,scales,offsets,status, $
          silent=silent, tempdir=tempdir, unsigned=unsigned
	
        if status ge 0 and rsize gt 0 then $
          mrd3_read_image,unit,arange,maxd,rsize, table
        size = rsize
        ...
        mrd3_ascii, header, structyp, use_colnum,                             $
        ...
        if status ge 0   and  size gt 0  then begin
            ...
            ;*** Read data.
            mrd3_read_ascii, unit,  arange, nbytes, nrows,   $
              nfld, typarr, posarr, lenarr, nullarr, table, $
              old_struct=old_struct
              
            ;*** Extract desired columns.
            if status ge 0 and keyword_set(columns) then                  $
                mrd3_columns, table, columns, fnames, fvalues, vcls, vtps, $
                  scales, offsets,                                        $
        ...
        IF keyword_set(deja_vu) AND n_elements(tab) NE 0 THEN BEGIN 
            ;; how many objects?
        ...
        ENDIF ELSE BEGIN 
            mrd3_table, header, structyp, use_colnum,                   $
        ...
        if status ge 0  and  size gt 0  then begin
            ...
            if status ge 0 and n_elements(vcls) gt 0 then begin 
                ...
                mrd3_read_heap, unit, header, arange, fnames, fvalues,     $
    ...
    if  status ge 0  and  scaling  and  size gt 0  then begin
        ...
        ;*** Apply scalings.
        if w[0] ne -1 then mrd3_scale, type, scales, offsets, table, header,  $

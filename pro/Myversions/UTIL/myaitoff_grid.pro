...
PRO MYAITOFF_GRID,DLONG,DLAT,LABEL=LABEL, NEW = new, _EXTRA= E, $
                  CHARSIZE = charsize, CHARTHICK =charthick, $
                  longrange=longrange, latrange=latrange, $
                  lcolor=lcolor

	if  N_elements(dlong) EQ 0 then dlong = 30.0
	if  N_elements(dlat) EQ 0 then dlat = 30.0

	IF n_elements(longrange) EQ 0 THEN longrange = [0,360]
	IF n_elements(latrange) EQ 0 THEN latrange = [-90, 90]
	...
	if keyword_set(label) then begin
		...
		for i=0,2*lngtot-1 do begin
			...
			tlng = (360 + (i*dlong)) mod 360 
			lngname =  ntostr(long(tlng))
			print,lng,'   '+lngname
			;;lngname = strcompress(string(lng,format="(I4)"),/remove)
			if (lng ne 180.0) then begin
				aitoff,lng,0.0,x,y
				if label eq 1 AND (tlng LE longrange[1]) AND (tlng GE longrange[0]) then BEGIN 
					xyouts,x[0]+xoff,y[0]+1,$
						lngname, $
						charsize = charsize, charthick = charthick, $
						color=lcolor
				ENDIF else IF (tlng LE longrange[1]) AND (tlng GE longrange[0]) THEN begin
					tmp = tlng/15.
		...
		for i=0,lattot-2 do begin
			IF (slat[i] NE 0) AND (lat[i] LE latrange[1]) AND (lat[i] GE latrange[0]) THEN xyouts,x[i]+2,y[i]+1, slat[i], $
				charsize = charsize, charthick = charthick, color=lcolor
		endfor
	endif

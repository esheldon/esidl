...
pro mrd_ascii, header, structyp, use_colnum,   $
    ...
    if strmid(fxpar(header, 'XTENSION'),0,8) ne 'TABLE   ' then begin
        print, 'MRDFITS_TEST: Header is not from ASCII table.'
        status = -1;
        return
    endif
    ...
    if N_elements(rows) EQ 0 then nrows = range[1] - range[0] + 1 else begin
          ...
          if Nbad GT 0  then begin 
             print,'MRDFITS_TEST: Row numbers must be between 0 and ' + $
                      strtrim(nrows-1,2)
             status = -1
             return
          endif      
    ...
    if nrows le 0 then begin
        if not keyword_set(silent) then begin
            print,'MRDFITS_TEST: ASCII table.  ',strcompress(string(nfld)),  $
                  ' columns, no rows'
        endif
        return
    endif
	...
	if cnt eq 0 then fnull = ''
        ...
        print, 'MRDFITS_TEST: Invalid format code:',ftype, ' for column ', i+1
        status = -1
        return
  next_col:
    endfor
...
pro  mrd_columns, table, columns, fnames, fvalues, $
    ...
    if n_elements(tcols) le 0 then begin
        print, 'MRDFITS_TEST:  No columns match'
...
pro mrd_read_image, unit, range, maxd, rsize, table, rows = rows 
    ...
    8: table = table[*,*,*,*,*,*,*,row1]
          ...
          print,'MRDFITS_TEST: Subscripted image must be between 1 and 8 dimensions'
          status = -1
          return
          end
    endcase
    ...
    for i=naxis-1,1,-1 do begin 
        ...
        if dims[i] eq 1 then begin
            if not mysilent then begin
                print, 'MRDFITS_TEST: Truncating unused dimensions'
                mysilent = 1
            endif
            dims = dims[0:i-1] 
            naxis = naxis - 1 
...
pro mrd_image, header, range, maxd, rsize, table, scales, offsets, scaling, $
  status, silent=silent, unsigned=unsigned, rows = rows
	...
	print,'MRDFITS_TEST: Error: Invalid BITPIX: '+strtrim(bitpix)
	table = 0
	return
    endif
	...
	maxd = gcount
        ...
        if (n_elements(dims) gt 1) then begin
            ...
            print, 'MRDFITS_TEST: Warning: No data specified for group data.'
            dims = [0]
            naxis = 0
        endelse
	...
	if xunsigned then begin
        ...
        if naxis eq 0 then begin
            ...
            table = 0
            if not keyword_set(silent) then begin
                print, 'MRDFITS_TEST: Null image, NAXIS=0'
            endif
            return
        ...
        if not keyword_set(silent) then begin
            ...
            str = str+')'
            print, 'MRDFITS_TEST: Image array ',str, '  Type=', typstrs[type]
        endif
...
pro mrd_read_heap, unit, header, range, fnames, fvalues, vcls, vtpes, table, $ 
   ...
   columns=columns, rows = rows, pointer_var=pointer_var, fixed_var=fixed_var
    ...
    if heapoff ne 0 and heapoff lt sz then begin 
        print, 'MRDFITS_TEST: ERROR Heap begins within data area' 
        status = -1 
        return 
    endif
    ...
    if delete then begin
        ...
        if ww[0] ne -1 then  begin
            w[vcols[ww]] = 0
            if not keyword_set(silent) then begin
                print, 'MRDFITS_TEST: ', strcompress(string(n_elements(ww))),  $
        ...
        if (wx[0] eq -1) then begin
            if not keyword_set(silent) then begin
                print, 'MRDFITS_TEST: All columns have been deleted'
            endif
	    table = 0
	    return
        endif
    ...
    if not keyword_set(pointer_var) then begin
        ...
        if n_elements(lencols) gt 0 then begin
            if not keyword_set(silent) then begin
                print, 'MRDFITS_TEST: ', strcompress(string(n_elements(lencols))), $
...
pro mrd_tdim, header, index, flen, arrstr, no_tdim=no_tdim
    ...
    if strmid(tdstr,0,1) ne '(' and strmid(tdstr,len-1,1) ne ')' or len lt 3 then begin
        print, 'MRDFITS_TEST: Error: invalid TDIM for column', index
        return
    endif
    ...
    if prod ne flen then begin
        print,'MRDFITS_TEST: Error: TDIM/TFORM dimension mismatch'
        return
    endif
...
;
  ...
  ;; begin with the types.  Then try to extract along with it the
  ;; forms, scales, and offsets (TZERO)x
  nam = 'TTYPE*'
...
;
  ...
  ;; All our words have same length
...
;  a number.  Store the positions of the located keywords in mnames, and the
;  value of the number field in numnames
;


  keyword = STRMID( hdr, 0, 8)
  
  mnames = WHERE(STRPOS(keyword,nam) GE 0, n_mnames)

  IF ( n_mnames GT 0 ) THEN BEGIN
      numst= STRMID(hdr[mnames], name_length, num_length)
      number = INTARR(n_mnames)-1

      FOR i = 0, n_mnames-1 DO         $
        IF VALID_NUM( numst[i], num) THEN number[i] = num

      igood = WHERE(number GE 0, n_mnames)
      IF n_mnames GT 0 THEN BEGIN
          mnames = mnames[igood]
          number = number[igood]
          numst = numst[igood]
  ...
  ;; The others
  fnames = strarr(n_mnames)
  fforms = strarr(n_mnames) 
  scales = dblarr(n_mnames)
  offsets = dblarr(n_mnames)

  ;;comments = strarr(n_mnames)

  fforms_names  = 'TFORM'+numst
  scales_names  = 'TSCAL'+numst
  offsets_names = 'TZERO'+numst


  match, keyword, fforms_names, mkey_forms, mforms
  IF mforms[0] EQ -1 THEN n_mforms = 0 ELSE n_mforms=n_elements(mforms)
  match, keyword, scales_names, mkey_scales, mscales
  IF mscales[0] EQ -1 THEN n_mscales = 0 ELSE n_mscales=n_elements(mscales)
  match, keyword, offsets_names, mkey_offsets, moffsets
  IF moffsets[0] EQ -1 THEN n_moffsets = 0 ELSE n_moffsets=n_elements(moffsets)

  ;;help,mnames,mforms,mscales,moffsets
  ;;print,n_mnames,n_mforms,n_mscales,n_moffsets
...
;              comments[ number[i]-1 ] =  comment
          ...
          CASE names[in] OF
              'TTYPE': fnames[*] = strtrim(result, 2)
              'TFORM': fforms[*] = strtrim(result, 2)
              'TSCAL': scales[*] = result
              'TZERO': offsets[*] = result
              ELSE: message,'What?'
          ENDCASE 

;
;  Set the value of !ERR for the number of nmatches for vectors, or simply 0
;  otherwise.
;          
;
...
;
  ...
  ;;help,fnames,fforms,scales,offsets
...
; NEW WAY: E.S.S.
    ...
    if xten ne 'BINTABLE' and xten ne 'A3DTABLE' then begin 
        print, 'MRDFITS_TEST: ERROR - Header is not from binary table.' 
        nfld = 0 & status = -1 
        return 
    endif 
...
;    nrows = nrow
    ...
    if nrow le 0 then begin
        if not keyword_set(silent) then begin
            print, 'MRDFITS_TEST: Binary table. ', $
...
;        fnames[i] = fname
        ...
        if ftype eq 'P' then begin 
            ...
            if dim ne 0  and dim ne 1 then begin 
                print, 'MRDFITS_TEST: Invalid dimension for variable array column '+string(i+1) 
                status = -1 
                return 
            endif
            ...
            if strpos('LXBIJKAEDCM', vf) eq -1 then begin 
                print, 'MRDFITS_TEST: Invalid type for variable array column '+string(i+1) 
                status = -1 
                return 
            endif 
  ...
  next_col: 
    ...
    if N_zero GT 0 then begin
        ...
        if N_zero Eq nfld then begin
            print,'MRDFITS_TEST: Error - All fields have zero length'
            return
        endif
	
        for i=0, N_zero-1 do begin
	    print,'MRDFITS_TEST: Table column ' + fnames[zero[i]] + ' has zero length'
	endfor
    ...
    if not keyword_set(silent) then begin
        print, 'MRDFITS_TEST: Binary table. ',strcompress(string(nfld)), ' columns by ',  $
          strcompress(string(nrowp)), ' rows.'
        if n_elements(vcls) gt 0 then begin
                print, 'MRDFITS_TEST: Uses variable length arrays'
        endif
    endif
...
function mrdfits_test, file, extension, header,      $
        structyp = structyp,                    $
    ...
    if keyword_set(version) then begin
        print,'MRDFITS_TEST: Version '+mrd_version()+' Oct 17, 2003'
    endif
	...
	if keyword_set(version) then return, 0
        print, 'MRDFITS_TEST: Usage'
        print, '   a=mrdfits_test(file/unit, [extension, header], /version       $'
    ...
    if keyword_set(structyp) then begin
        ...
        if sz[1] ne 7 then begin
            print, 'MRDFITS_TEST: stucture type must be a string'
            return, 0
        endif
    endif
    ...
    sz = size(file)
    if (sz[0] ne 0) then begin
        print, 'MRDFITS_TEST: Vector input not supported'
        return, 0
    endif
    ...
    if sz[1] gt 0 and sz[1] lt 4 then begin
        ...
        if unit lt 0 then begin
            print, 'MRDFITS_TEST: File access error'
            status = -1
            return, 0
        endif
    endelse
    ...
    if eof(unit) then begin
        print,'MRDFITS_TEST: Extension past EOF'
    ...
    if status lt 0 then begin
        print, 'MRDFITS_TEST: Unable to read header for extension'
	if inputUnit eq 0 then begin
            free_lun, unit
	endif
        return, 0
	...
	type = 2
        ...
        print, 'MRDFITS_TEST: Unable to process extension type:', xten
	if inputUnit eq 0 then begin
	    free_lun,unit
	endif
	status = -1

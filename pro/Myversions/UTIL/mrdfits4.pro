...
;
; CALLING SEQUENCE:
;      Result = MRDFITS3( Filename/FileUnit,[Extension, Header],
...
;                      repeat begin
;                          thisHDU = mrdfits4(lun, 0, hdr, status=status)
;                          ... process the HDU ...
...
;       Modified version 2.6: Added /deja_vu keyword, which allows the
;            user to reuse the structure created on the previous call for
;            binary tables. Saves factor of 10 in time for big structs. Search
;            below for Erin Sheldon and you will find the two changes. Also, 
;            to avoid conflicts with future changes in mrdfits functions, 
;            names were changed to mrdfits4*
;            Erin Sheldon, Dave Johnston. 25-Nov-2002 UofChicago
;
...
;=============================================================================
function mrd4_substruct, table, column
;
...
function mrd4_getc, table, column
    ...
    ; in a structure.
    if not mrd4_substruct(table, column) then begin
       return, table.(column)  
    endif else begin
...
function mrd4_sizcol, table, column
...
pro mrd4_putc, table, column, value
...
function  mrd4_dofn, name, index, use_colnum, alias=alias
...
function mrd4_chkfn, name, namelist, index 
...
function mrd4_unsigned_offset, type
...
function mrd4_chkunsigned, bitpix, scale, zero, unsigned=unsigned
...
function mrd4_unsignedtype, data
...
; Return the currrent version string for MRDFITS4
function mrd4_version
    return, '2.6'
end
;=====================================================================
...
pro mrd4_read_ascii, unit, range, nbytes, nrows, nfld, typarr, posarr,   $
	    ...
	    endelse
            ...
            if typarr[i] eq 'I' then begin
                if (not keyword_set(old_struct)) then begin
                    ...
                    mrd4_putc, table, i, long(flds)
                endelse
            endif else if typarr[i] eq 'E' or typarr[i] eq 'F' then begin
                if (not keyword_set(old_struct)) then begin
                    table.(i) = float(flds)
                endif else begin
                    mrd4_putc, table, i, float(flds)
                endelse
            endif else if typarr[i] eq 'D' then begin
                if (not keyword_set(old_struct)) then begin
                    table.(i) = double(flds)
                endif else begin
                    mrd4_putc, table, i, double(flds)
                endelse
            endif else if typarr[i] eq 'A' then begin
                if (not keyword_set(old_struct)) then begin
                    table.(i) = flds
                endif else begin
                    mrd4_putc, table, i, flds
                endelse
            endif
        endelse
    endfor
...
pro mrd4_ascii, header, structyp, use_colnum,   $
    range, table, $
    ...
    if strmid(fxpar(header, 'XTENSION'),0,8) ne 'TABLE   ' then begin
        print, 'MRDFITS4: Header is not from ASCII table.'
        status = -1;
        return
    endif
    ...
    if nrows le 0 then begin
        if not keyword_set(silent) then begin
            print,'MRDFITS4: ASCII table.  ',strcompress(string(nfld)),  $
                  ' columns, no rows'
        endif
        return
    endif
	...
	if cnt eq 0 then fnull = ''
        ...
        fname = mrd4_dofn(fname,i+1, use_colnum, alias=alias)
        fnames[i] = fname

        fname = mrd4_chkfn(fname, fnames, i)
        
        mrd4_atype, fform, ftype, flen
        typarr[i] = ftype
        lenarr[i] = flen
        posarr[i] = fpos
        nullarr[i] = fnull
        ...
        print, 'MRDFITS4: Invalid format code:',ftype, ' for column ', i+1
        status = -1
        return
  next_col:
    endfor
...
; user specification.
pro  mrd4_columns, table, columns, fnames, fvalues, $
    ...
    if n_elements(tcols) le 0 then begin
        print, 'MRDFITS4:  No columns match'
        ...
        for i=0, n_elements(tcols)-1 do begin
            if  not keyword_set(old_struct) then begin
                ...
                mrd4_putc, tabx, i, mrd4_getc(table,tcols[i])
            endelse
        endfor
...
pro mrd4_read_image, unit, range, maxd, rsize, table 
    ...
    type = mrd4_unsignedtype(table)
    if type gt 0 then begin
	table = table - mrd4_unsigned_offset(type)
    endif
...
pro mrd4_axes_trunc,naxis, dims, silent
    ...
    for i=naxis-1,1,-1 do begin 
        ...
        if dims[i] eq 1 then begin
            if not mysilent then begin
                print, 'MRDFITS4: Truncating unused dimensions'
                mysilent = 1
            endif
            dims = dims[0:i-1] 
            naxis = naxis - 1 
...
pro mrd4_image, header, range, maxd, rsize, table, scales, offsets, scaling, $
  status, silent=silent, tempdir=tempdir, unsigned=unsigned
    ...
    xunsigned = mrd4_chkunsigned(bitpix,  xscale, $
	...
	print,'MRDFITS4: Error: Invalid BITPIX: '+strtrim(bitpix)
	table = 0
	return
    endif
	...
	maxd = gcount
        ...
        if (n_elements(dims) gt 1) then begin
            ...
            print, 'MRDFITS4: Warning: No data specified for group data.'
            dims = [0]
            naxis = 0
        endelse
        ...
        mrd4_axes_trunc, naxis, dims, keyword_set(silent)
        ...
        if not keyword_set(silent) then print,'MRDFITS4--Image with groups:', $
	...
	if xunsigned then begin
	    fxaddpar,header, 'BZERO', 0, 'Reset by MRDFITS4 v'+mrd4_version()
	endif
        ...
        if naxis eq 0 then begin
            ...
            table = 0
            if not keyword_set(silent) then begin
                print, 'MRDFITS4: Null image, NAXIS=0'
            endif
            return
        ...
        if not keyword_set(silent) then begin
            ...
            str = str+')'
            print, 'MRDFITS4: Image array ',str, '  Type=', typstrs[type]
        endif
	...
	if xunsigned then begin
	    fxaddpar,header, 'BZERO', 0, 'Updated by MRDFITS4 v'+mrd4_version()
	endif
...
pro mrd4_ptrscale, array, scale, offset
...
pro mrd4_scale, type, scales, offsets, table, header,  $
	...
	; Just copy the unscaled columns
        if ww[0] ne -1 then begin
            ...
            for i=0, n_elements(ww)-1 do begin
                if (not keyword_set(old_struct)) then begin
                    ...
                    mrd4_putc, tabx, ww[i], mrd4_getc(table,ww[i])
                endelse
            endfor
        endif
		...
		sz = size(tabx.(w[i]))
		if sz[sz[0]+1] eq 10 then begin
		    mrd4_ptrscale, table.(w[i]), scales[w[i]], offsets[w[i]]
		endif
                tabx.(w[i]) = table.(w[i])*scales[w[i]] + offsets[w[i]]
		
            endif else begin
		
		sz = mrd4_sizcol(table, w[i])
		if sz[sz[0]+1] eq 10 then begin
		    arr = mrd4_getc(table,w[i])
		    mrd4_ptrscale, arr, scales[w[i]], offsets[w[i]]
		    mrd4_putc, tabx, w[i], arr
		endif else begin
                    mrd4_putc, tabx, w[i], mrd4_getc(table,w[i])*scales[w[i]] + offsets[w[i]]
		endelse
		
            endelse
            istr = strcompress(string(w[i]+1), /remo)
            fxaddpar, header, 'TSCAL'+istr, 1.0, 'Set by MRD4_SCALE'
            fxaddpar, header, 'TZERO'+istr, 0.0, 'Set by MRD4_SCALE'
        endfor
    ...
    ; Now process images and random groups.
        ...
        if sz[sz[0]+1] ne 8 then begin
            ...
            endelse
            fxaddpar, header, 'BSCALE', 1.0, 'Set by MRD4_SCALE'
            fxaddpar, header, 'BZERO', 0.0, 'Set by MRD4_SCALE'
            ...
            for i=0, nparam-1 do begin
                istr = strcompress(string(i+1),/remo)
                fxaddpar, header, 'PSCAL'+istr, 1.0, 'Added by MRD4_SCALE'
                fxaddpar, header, 'PZERO'+istr, 0.0, 'Added by MRD4_SCALE'
                tabx.(0)[i] = table.(0)[i]*scales[i]+offsets[i]
            endfor
	    
            tabx.(1) = table.(1)*scales[nparam] + offsets[nparam]
            fxaddpar, header, 'BSCALE', 1.0, 'Added by MRD4_SCALE'
            fxaddpar, header, 'BZERO', 0.0, 'Added by MRD4_SCALE'
            table = temporary(tabx)
        endelse
    endelse
...
pro mrd4_varcolumn, vtype, array, heap, off, siz
    ...
    unsigned = mrd4_unsigned_offset(unsigned)
...
pro mrd4_fixcolumn, vtype, array, heap, off, siz
...
; length arrays. 
pro mrd4_read_heap, unit, header, range, fnames, fvalues, vcls, vtpes, table, $ 
   ...
   old_struct=old_struct, pointer_var=pointer_var, fixed_var=fixed_var
    ...
    if heapoff ne 0 and heapoff lt sz then begin 
        print, 'MRDFITS4: ERROR Heap begins within data area' 
        status = -1 
        return 
    endif
    ...
    if delete then begin
        ...
        if ww[0] ne -1 then  begin
            w[vcols[ww]] = 0
            if not keyword_set(silent) then begin
                print, 'MRDFITS4: ', strcompress(string(n_elements(ww))),  $
        ...
        if (wx[0] eq -1) then begin
            if not keyword_set(silent) then begin
                print, 'MRDFITS4: All columns have been deleted'
            endif
	    table = 0
	    return
        endif
    ...
    if not keyword_set(pointer_var) then begin
        ...
        if n_elements(lencols) gt 0 then begin
            if not keyword_set(silent) then begin
                print, 'MRDFITS4: ', strcompress(string(n_elements(lencols))), $
    ...
    for i=0, n_elements(fnames)-1 do begin
        ...
        if col ge 0 then begin
            ...
            if w[0] eq -1 then begin
                ...
                if not keyword_set(old_struct) then begin
                    ...
                    mrd4_putc, tablex, i, mrd4_getc(table,col)
                endelse
		...
		if varying and keyword_set(pointer_var) and vtype ne 'A' then begin
		    mrd4_varcolumn, vtype, curr_colx, heap, off, siz
		endif else begin
		    mrd4_fixcolumn, vtype, curr_colx, heap, off, siz
		endelse
            ...
            if not keyword_set(old_struct) then begin
                ...
                xx = mrd4_getc(table, ncol)
                mrd4_putc, tablex, i, reform(xx[0,*])
            endelse
        endelse
    endfor 
...
pro mrd4_read_table, unit, range, rsize, structyp, nrows, nfld, typarr, table, $
                                      old_struct=old_struct
  ...
  nxtfld: 
    ...
    for i=0, nfld-1 do begin
	    ...
	    type = mrd4_unsignedtype(table.(i))

	    if type gt 0 then begin
	        table.(i) = table.(i) - mrd4_unsigned_offset(type)
	    endif
	    
	endif else begin
	    
	    col = mrd4_getc(table,i)
	    type  = mrd4_unsignedtype(col)
	    
	    if type gt 0 then begin
		mrd4_putc, table, i, col - mrd4_unsigned_offset(type)
	    endif
...
pro mrd4_tdim, header, index, flen, arrstr, no_tdim=no_tdim
    ...
    if strmid(tdstr,0,1) ne '(' and strmid(tdstr,len-1,1) ne ')' or len lt 3 then begin
        print, 'MRDFITS4: Error: invalid TDIM for column', index
        return
    endif
    ...
    if prod ne flen then begin
        print,'MRDFITS4: Error: TDIM/TFORM dimension mismatch'
        return
    endif
...
pro mrd4_table, header, structyp, use_colnum,           $ 
    ...
    if xten ne 'BINTABLE' and xten ne 'A3DTABLE' then begin 
        print, 'MRDFITS4: ERROR - Header is not from binary table.' 
        nfld = 0 & status = -1 
        return 
    endif 
    ...
    for i=0, nfld-1 do begin
        ...
        if dim eq 1 then begin
            ...
            mrd4_tdim, header, i+1, dim, str, no_tdim=no_tdim
            dimfld[i] = str
        endelse
        ...
        if ftype eq 'P' then begin 
            ...
            if dim ne 0  and dim ne 1 then begin 
                print, 'MRDFITS4: Invalid dimension for variable array column '+string(i+1) 
                status = -1 
                return 
            endif
            ...
            if strpos('LXBIJKAEDCM', vf) eq -1 then begin 
                print, 'MRDFITS4: Invalid type for variable array column '+string(i+1) 
                status = -1 
                return 
            endif 
	    ...
	    xunsigned = mrd4_chkunsigned(bitpix[ppos], xscale,       $
		...
		xunsigned = mrd4_chkunsigned(bitpix[j], xscale, $
					   fxpar(header, 'TZERO'+istr), $
					   unsigned=unsigned)

		if xunsigned then begin
		    fxaddpar, header, 'TZERO'+istr, 0, 'Modified by MRDFITS4 V'+mrd4_version()
		endif
  ...
  next_col: 
    ...
    if N_zero GT 0 then begin
        ...
        if N_zero Eq nfld then begin
            print,'MRDFITS4: Error - All fields have zero length'
            return
        endif
	
        for i=0, N_zero-1 do begin
	    print,'MRDFITS4: Table column ' + fnames[zero[i]] + ' has zero length'
	endfor
    ...
    if not keyword_set(silent) then begin
        print, 'MRDFITS4: Binary table. ',strcompress(string(nfld)), ' columns by ',  $
          strcompress(string(nrow)), ' rows.'
        if n_elements(vcls) gt 0 then begin
                print, 'MRDFITS4: Uses variable length arrays'
        endif
    endif
...
FUNCTION mrd4_create_mrdfits_struct, tab, $
                                     rsize, $
                                     nfld, fnames, fvalues, typarr,$
                                     scales, offsets, $
                                     vcls, vtpes


  IF n_elements(vcls) NE 0 THEN u_vcls = vcls ELSE u_vcls = -1
  IF n_elements(vtpes) NE 0 THEN u_vtpes = vtpes ELSE u_vtpes = '-1'
  
  mrdfits_struct = create_struct('tab', tab, $
                                 'rsize', rsize, $
                                 'nfld', nfld, $
                                 'fnames', fnames, $
                                 'fvalues', fvalues, $
  ...
  return, mrdfits_struct

END 

PRO mrd4_copy_mrdfits_struct, mrdfits_struct, $
                              tab, $
                              rsize, $
                              nfld, fnames, fvalues, typarr,$
                              scales, offsets, $
      ...
      tab = mrdfits_struct.tab
      rsize = mrdfits_struct.rsize
      nfld = mrdfits_struct.nfld
      typarr = mrdfits_struct.typarr
      fnames = mrdfits_struct.fnames
      fvalues = mrdfits_struct.fvalues
      IF mrdfits_struct.vcls[0] NE -1 THEN vcls = mrdfits_struct.vcls
      IF mrdfits_struct.vtpes[0] NE '-1' THEN vtpes = mrdfits_struct.vtpes
      scales = mrdfits_struct.scales
      offsets = mrdfits_struct.offsets
  
END 

function mrdfits4, file, extension, header,               $
                  mrdfits_struct=mrdfits_struct,          $
                  structyp = structyp,                    $
    ...
    ;   Let user know version if MRDFITS4 being used.
    if keyword_set(version) then begin
        print,'MRDFITS4: Version '+mrd4_version()+' February 18, 2002'
    endif
	...
	if keyword_set(version) then return, 0
        print, 'MRDFITS4: Usage'
        print, '   a=mrdfits4(file/unit, [extension, header], /version       $'
    ...
    if keyword_set(structyp) then begin
        ...
        if sz[1] ne 7 then begin
            print, 'MRDFITS4: stucture type must be a string'
            return, 0
        endif
    endif
    ...
    sz = size(file)
    if (sz[0] ne 0) then begin
        print, 'MRDFITS4: Vector input not supported'
        return, 0
    endif
    ...
    if sz[1] gt 0 and sz[1] lt 4 then begin
        ...
        if unit lt 0 then begin
            print, 'MRDFITS4: File access error'
            status = -1
            return, 0
        endif
    endelse
    ...
    if eof(unit) then begin
        print,'MRDFITS4: Extension past EOF'
    ...
    if status lt 0 then begin
        print, 'MRDFITS4: Unable to read header for extension'
	if inputUnit eq 0 then begin
            free_lun, unit
	endif
        return, 0
	...
	type = 2
        ...
        print, 'MRDFITS4: Unable to process extension type:', xten
	if inputUnit eq 0 then begin
	    free_lun,unit
	endif
	status = -1
    ...
    if type eq 0 then begin
        ...
        mrd4_image,header,arange,maxd,rsize,table,scales,offsets,status, $
          silent=silent, tempdir=tempdir, unsigned=unsigned
	
        if status ge 0 and rsize gt 0 then $
          mrd4_read_image,unit,arange,maxd,rsize, table
        size = rsize
        ...
        mrd4_ascii, header, structyp, use_colnum,                             $
        ...
        if status ge 0   and  size gt 0  then begin
            ...
            ;*** Read data.
            mrd4_read_ascii, unit,  arange, nbytes, nrows,   $
              nfld, typarr, posarr, lenarr, nullarr, table, $
              old_struct=old_struct
              
            ;*** Extract desired columns.
            if status ge 0 and keyword_set(columns) then                  $
                mrd4_columns, table, columns, fnames, fvalues, vcls, vtps, $
                  scales, offsets,                                        $
        ...
        ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
        ;; Added Erin Sheldon (Dave Johnston) 25-Nov-2002
        ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

        IF n_elements(mrdfits_struct) NE 0 THEN BEGIN 


            ;; how many rows?
            ;; this should be all that changes; if not the user
            ;; has messed up.
            nrows = fxpar(header, 'NAXIS2')
            IF nrows EQ 0 THEN BEGIN 
                print,"MRDFITS4:       EXTENSION IS EMPTY"
                return, 0
            ENDIF
            arange = [0, nrows -1]
            
            ;; get the saved values from mrdfits_struct
            mrd4_copy_mrdfits_struct, mrdfits_struct, $
                                      tab, $
                                      rsize, $
                                      nfld, fnames, fvalues, typarr,$
                                      scales, offsets, $
            ...
            table = replicate(tab, nrows)
        ENDIF ELSE BEGIN 
            mrd4_table, header, structyp, use_colnum,                   $
                       arange, rsize, table, nrows, nfld, typarr,       $ 
                       fnames, fvalues, vcls, vtpes, $
                       scales, offsets, scaling, status,   $
                       silent=silent, tempdir=tempdir, $
                       columns=columns, no_tdim=no_tdim, $
                       old_struct=old_struct, alias=alias, unsigned=unsigned

            
            mrdfits_struct = $
              mrd4_create_mrdfits_struct(table[0], $
                                         rsize, $
                                         nfld, fnames, fvalues, typarr,$
                                         scales, offsets, $
                                         vcls, vtpes)
        ...
        if status ge 0  and  size gt 0  then begin
            ...
            if status ge 0 and n_elements(vcls) gt 0 then begin 
                ...
                mrd4_read_heap, unit, header, arange, fnames, fvalues,     $
    ...
    if  status ge 0  and  scaling  and  size gt 0  then begin
        ...
        ;*** Apply scalings.
        if w[0] ne -1 then mrd4_scale, type, scales, offsets, table, header,  $

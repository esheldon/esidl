PRO binfield, clr, runs, get=get,stars=stars,addf=addf,log=log,$
             hist=hist
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  IF N_params() EQ 0 THEN BEGIN 
     print,'-Syntax: binfield, clr, runs, get=get,stars=stars'
     print,''
    ...
    FOR i=0,ncol-1 DO BEGIN
      ...
      IF i EQ 0 THEN BEGIN
        ...
        ENDIF ELSE BEGIN 
          ...
          w52=w52[0:nfield-1]
        ENDELSE 
        nfield = nfield-1 ; makes things match up better 
        runs=replicate(r52[0], 2*ncol, nfield) 

        print,w52[1]+1,w52[nfield]+1
        print,w56[0]+1,w56[nfield-1]+1
      ENDIF 
      
      w56=where( r56.ra GT minra, n56)
      w52=where( r52.ra LT maxra, n52)
      
      r52=r52[w52[1:nfield]]
      ...
      FOR cc=0, nc-1 DO BEGIN
        FOR ff=0, nf-1 DO BEGIN 
          ...
          IF csize EQ 1 AND fsize EQ 1 THEN BEGIN 
            ; Binsize is 1x1
            te1 = runs[cc,ff].e1[clr]
            te2 = runs[cc,ff].e2[clr]
            te1err = runs[cc,ff].e1err[clr]
            te2err = runs[cc,ff].e2err[clr]
            te1sig = runs[cc,ff].e1sig[clr]
            te2sig = runs[cc,ff].e2sig[clr]
         ...
         ENDIF ELSE BEGIN 
            ...
            te1r = runs[cc*csize:cc*csize+cstep, $
                              ff*fsize:ff*fsize+fstep].e1[clr]
            te2r = runs[cc*csize:cc*csize+cstep, $
                              ff*fsize:ff*fsize+fstep].e2[clr]
            te1errr = runs[cc*csize:cc*csize+cstep, $
                              ff*fsize:ff*fsize+fstep].e1err[clr]
            te2errr = runs[cc*csize:cc*csize+cstep, $
                              ff*fsize:ff*fsize+fstep].e2err[clr]
            te1sigr = runs[cc*csize:cc*csize+cstep, $
                              ff*fsize:ff*fsize+fstep].e1sig[clr]
            te2sigr = runs[cc*csize:cc*csize+cstep, $
                              ff*fsize:ff*fsize+fstep].e2sig[clr]

PRO  red_fit, cZ, sigma, mag, coeffs, predicted, degree

;*****************************************************************************
;  Procedure:  red_fit
;  
;  Purpose:  Find a first or second degree polynomial fit to redshift data
;            as a function of the magntudes r=X[1],g=X[2],u=X[3],
;            i=X[4],z=X[5]. 
;       
;  Inputs:  cZ:        an array of redshift values of length Num2
;	    sigma:    the standard dev. for each value of cZ
;           mag:        the magnitudes for each cZ. mag=dblarr[6,Num2]
...
;*****************************************************************************
  ...
  IF N_PARAMS() LT 2 THEN BEGIN
    PRINT,"Syntax - red_fit, cZ, mag, sigma, coeffs, predicted, degree"
    return
  ENDIF 
...
;*****************************************************************************
  ...
  Num2 = size(cZ)
  IF n_elements(sigma) EQ 0 THEN sigma = dblarr(Num2) + 1.0

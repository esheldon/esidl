...
c     interpolates weights for poorly sampled objects
      ...
      real tol,xinterp2,xl,xh,yl,yh,xxx,yyy,xy,td,e1,e2,xinterp
      integer maxit
      ...
      real wcenx(nel),wceny(nel),e1old,e2old,ymod
      logical interpflag
      parameter(tol=0.01,maxit=100,xinterp=3.0,xinterp2=xinterp*xinterp)
...
c     rmax=max(rmax,xx2+yy2)
     ...
     &    abs(ycen-ycenorig).gt.shiftmax)then
          ...
          e1=(w(1,1)-w(2,2))/td
          e2=2.*w(1,2)/td
          if(abs(e1-e1old).lt.tol.and.abs(e2-e2old).lt.tol)then
            ixx(kk)=w(1,1)
            iyy(kk)=w(2,2)
            ixy(kk)=w(1,2)
            rho4(kk)=sums4/sum

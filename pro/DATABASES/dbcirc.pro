function dbcirc, ra_cen, dec_cen, radius, dis, sublist,SILENT=silent, $
...
;
; INPUTS:
;       RA_CEN - Right ascension of the search center in decimal DEGREES scalar
...
;-                   
 ...
 if keyword_set(TO_J2000) then begin
        jprecess,ra_cen,dec_cen,racen,deccen 
        racen = racen[0]    &       deccen = deccen[0]
 endif else  if keyword_set(TO_B1950) then begin
        bprecess,ra_cen,dec_cen,racen,deccen 
        racen = racen[0]    &       deccen = deccen[0]
 endif else begin
 ...
 if abs(bigdec) EQ 90 then rasize = 360. else $             ;Updated Sep 1999
       rasize = abs( size/cos(bigdec/!RADEG) ) < 360.  ;Correct for latitude effect

 if 2*rasize gt 360. then begin         ;Only need search on Dec?
...
; Use GCIRC to compute angular distance of each source to the field center
 ...
 if max(list) GT 0 then begin                         ;Any entries found?
        ...
        gcirc,0, racen/!RADEG, deccen/!RADEG, ra_match/!RADEG, dec_match/!RADEG, ddis
        ;; convert ddis to degrees
        ddis = ddis*!RADEG
        good = where( ddis LT size, Nfound )

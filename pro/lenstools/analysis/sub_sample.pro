...
PRO sub_sample, lensumfile, randsumfile, wstr, outdir=outdir, addstr=addstr_in, indices=indices, uselens=uselens, userand=userand, addweight=addweight, overwrite=overwrite

  nparam = n_params()
  IF nparam LT 2 THEN BEGIN 
      print,'-Syntax: sub_sample, lensumfile, randsumfile, wstr, outdir=outdir, addstr=addstr,indices=indices,uselens=uselens, userand=userand, addweight=addweight'
      return
  ENDIF 
  IF nparam LT 3 THEN BEGIN 
      IF n_elements(indices) EQ 0 THEN BEGIN
          print,'-Syntax: sub_sample, lensumfile, randsumfile, wstr, outdir=outdir, addstr=addstr,indices=indices'
          return
      ENDIF 
  ENDIF 
  ...
  combine_lensum, randsum, binsize, rmin, rmax, rndsumst, rndshst
  ...
  rndshhdr = shhdr(rndshst)

  ;; Sum file for random
  rndsumhdr = sumhdr(rndsumst)
  ...
  print,lshf
  mwrfits2, shst, lshf, shhdr, /create, /destroy
  print,lsumf
  mwrfits2, sumst, lsumf, sumhdr, /create, /destroy
  print,lf
  mwrfits2, lensum, lf, lensumhdr, /create, /destroy

  print
  print,rshf
  mwrfits2, rndshst, rshf, rndshhdr, /create, /destroy
  print,rsumf
  mwrfits2, rndsumst, rsumf, rndsumhdr, /create, /destroy
  print,rf

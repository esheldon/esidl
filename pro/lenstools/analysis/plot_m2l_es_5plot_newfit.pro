...
PRO plot_m2l_es_5plot_newfit, $
                              radbin, stripes=stripes, doallow=doallow, $
                              check=check, readfromfile=readfromfile, $
                              neighborlum=neighborlum,$
      ...
      FOR i=0L,nlum-1 DO BEGIN
          ...
          FOR clr=0L, nclr-1 DO BEGIN 
              ...
              IF keyword_set(neighborlum) THEN BEGIN 
                  ...
                  la=lumalpha[clr]
                  lumdens=lumnorm[clr]*xxn^(-la) ;10^10 Lsun/Mpc^2
                  ;; convert to mean lumdens within radius, then multiply by area
                  neighlumin = lumdens*2./(2.-la)*!pi*xxn^2
                  print,'Cent = ',tave/1.e10,' Neigh = ',neighlumin[ww[nww-1]]
                  lumtot = tave + neighlumin*1.e10 ;lum
                  tave = lumtot[ww[nww-1]]
              ENDIF 
  ...
  IF keyword_set(nocolor) THEN BEGIN 
      ...
      lines = [0,1,2,3,4]
  ENDIF ELSE BEGIN 
      pcolor=[!blue,!cyan,!green,!magenta,!red]
      typecolor=[!blue,!black,!red]
      lines = replicate(0,5)
  ENDELSE 
  
  IF keyword_set(neighborlum) THEN BEGIN 
;      xtitle='L(<'+rmax_actstr[radbin]+' kpc)  (h!U'+!tsym.minus+'2!N 10!U10!N L!DSun!N)'
;      ytitle = 'M/L(< '+rmax_actstr[radbin]+' kpc) (h M!DSun!N/L!DSun!N)'
      xtitle='L(<'+rmax_actstr[radbin]+' kpc)  (h!U'+!tsym.minus+'2!N 10!U10!N L'+sunsymbol()+')'
      ytitle = 'M/L(< '+rmax_actstr[radbin]+' kpc) (h M'+sunsymbol()+' /L'+sunsymbol()+')'
  ENDIF ELSE BEGIN 
...
;tnormmax = 300.
  FOR clr=0L,nclr-1 DO BEGIN 
      ...
      CASE clr OF
          ...
          1:BEGIN & normmax=tnormmax & normmin=0.0 & powmin=0. & powmax=2.0 & END
...
;          addtit=' (h M!DSun!N/L!DSun!N)'
          ...
          ttyytt=!tsym.upsilon_cap+addtit
          ttxxtt=!tsym.beta
          doxaxis=1
      ENDIF 
      IF clr EQ 3 THEN BEGIN
          ...
          doxaxis=1
      ENDIF
      IF clr GT 0 THEN multiplot;,doxaxis=doxaxis
      ...
      pow_chisq_conf,lum[clr,*],mass[clr,*]*100.,masserr[clr,*]*100.,$
                     ...
                     xtitle=ttxxtt,ytitle=ttyytt,$
                     /center,charsize=chsz, xtick_get=chxtick_get,xrange=[0,2]
      bestpow[clr] = bestp
      bestnorm[clr] = bestn
      ...
      IF clr EQ 3 THEN BEGIN 
          nch=n_elements(chxtick_get)
          chtick=strarr(nch)
          chtick[0] = ' '
          chtick[1:nch-1] = ntostr(chxtick_get[1:nch-1],3)
          axis,xaxis=0, xtickname=chtick
      ENDIF 
...
;      IF (clr EQ 1) OR (clr EQ 0) THEN xrg=[0, 4] $
;      ELSE IF clr EQ 3 THEN xrg=[0,15] ELSE delvarx,xrg
      ...
      CASE clr OF
          0: pos = [ [px1, py1], [px2, py2] ]
          1: pos = [ [px1+dx, py1], [px2+dx, py2] ]
          2: pos = [ [px1, py1-dy], [px2, py2-dy] ]
          3: pos = [ [px1+dx, py1-dy], [px2+dx, py2-dy] ]
          4: pos = [ [px1, py1-2.*dy], [px2, py2-2.*dy] ]
      ENDCASE 
      ...
      ploterror,lum[clr,*],mass[clr,*],masserr[clr,*],$
                ...
                xtitle='L [10!U10!N L'+sunsymbol()+']',$
                ytitle='M [10!U12!N M'+sunsymbol()+']',$
                xtick_get=vx,ytick_get=vy,$
        ticklen=.04

      ;; deal with axis (multiplot screws up stuff)
      ;;X-axis

      IF clr NE 4 THEN BEGIN 
          nv=n_elements(vx)
          vxstr=strarr(nv)
          w=where(vx LT 1,nw)
          IF nw NE 0 THEN vxstr[w] = ntostr(vx[w],3)
          w=where((vx GT 1) OR (vx EQ 0),nw)
          IF nw NE 0 THEN vxstr[w] = ntostr(long(rnd(vx[w])),2)
          axis,xaxis=0,xtickname=vxstr,charsize=1
      ENDIF 
      IF (clr EQ 1) OR (clr EQ 3) THEN BEGIN 
          nv=n_elements(vy)
          vystr=strarr(nv)
          w=where(vy LT 1,nw)
          IF nw NE 0 THEN vystr[w] = ntostr(vy[w],3)
          w=where((vy GT 1) OR (vy EQ 0),nw)
          IF nw NE 0 THEN vystr[w] = ntostr(long(rnd(vy[w])),2)
          axis,yaxis=0,ytickname=vystr,charsize=1
      ENDIF 
...
;  colprint,mass[0,*]/masserr[0,*], mass[1,*]/masserr[1,*], mass[2,*]/masserr[2,*], mass[3,*]/masserr[3,*], mass[4,*]/masserr[4,*]
          ...
          FOR clr=0L, nclr-1 DO BEGIN 
              ...
              IF keyword_set(redolum) THEN BEGIN 
                  ...
                  IF i EQ 1 THEN BEGIN 
                      ;; get stuff classified
                      make_clflag_struct,fs
                      fs.failed='N'
                      fs.NOT_a_galaxy = 'N'
                      fs.ellip = 'Y'
                      clflag_select,lensum,fs,si1
                      fs.ellip = 'D'
                      fs.spiral = 'Y'
                      clflag_select,lensum,fs,si2
                      si = [si1,si2]
                      lensum=lensum[si]
                  ENDIF 
              ENDIF 
              ...
              IF keyword_set(neighborlum) THEN BEGIN 
                  ...
                  neighlumin = lumdens*2./(2.-la)*!pi*xxn^2
                  print,types[i]+' Cent = ',tave/1.e10,' Neigh = ',$
                    neighlumin[w[nww-1]]
                  lumtot = tave + neighlumin*1.e10 ;lum
                  tave = lumtot[w[nww-1]]
              ENDIF 
  ...
  legend,['Late Type','All Galaxies','Early Type'],symsize=symsize,$
  ...
  FOR clr=0L,nclr-1 DO BEGIN 
      ...
      IF clr EQ 1 THEN BEGIN 
          myusersym,'fill_triangle'
          legend,['Late Type','Early Type'],symsize=symsize,thick=[!p.thick,!p.thick],$
  ...
  FOR clr=0L,nclr-1 DO BEGIN 
      ...
      ENDIF ELSE BEGIN 
          ...
          left=1
      ENDELSE 
      legend,['Late Type','All Galaxies','Early Type'],symsize=symsize,thick=lthick,$

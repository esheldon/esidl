PRO wtheta_absmag_meangr, lz, cindex, mag, gmr, absmag, lum_solar,kcorr=kcorr;, old=old

  IF n_params() EQ 0 THEN BEGIN 
      print,'-Syntax: wtheta_absmag_meangr, lz, cindex, mag, gmr, absmag, lum_solar, kcorr=kcorr'
      return
  ENDIF 
  ...
  medgr = median(gmr)
  type = interpol(findgen(6), carr, medgr) > 0. < 5.

  kcorr = -1.0*interpolate(tkarray, lz/0.05, type)

pro fitmom_retmom, run, rerun, camcol, maxmag=maxmag,$
...
;  newfront = 'adatc'
  ...
  FOR ic=0, nframes-1 DO BEGIN 
      ...
      ENDIF 
      ;wg=where( (pstruct.ixx[1] NE 0.) OR $
      ;          (pstruct.ixx[2] NE 0.) OR $
      ;          (pstruct.ixx[3] NE 0.) ,ng)
      IF ng NE 0. THEN BEGIN 
          print
...
;                  e2 = 2.*ixy/(ixx+iyy)
                  ...
                  ;; using weight of object
                      ixx = (admomstruct[ind2].ixx[cindex]+$
                             admomstruct[ind2].iyy[cindex])/2.0
                      iyy = ixx
                      ixy = 0.0
;                  gale1 = (ixx-iyy)/(ixx+iyy)
;                  gale2 = 2.*ixy/(ixx+iyy)
                      tmp = call_external(value = [0B,0B,0B,0B,0B,0B,0B,0B,0B],$
                                          bindir+'psfadmom_retmom.so','psfadmom_retmom', $
                                          *psf[iband], nrow, ncol,$
                                          ixx, iyy, ixy, momerr, rho4, whyflag)
...
;                  colprint,[e1,e2],[ge1,ge2],[gale1,gale2]
;                  print

...
pro corshape_allcols_retmom, run, rerun, clr, start=start, nframes=nframes,$
                              minmag=minmag, maxmag=maxmag, $
                              rcut=rcut, ermscut=ermscut, $
                              errmax=errmax, psfile=psfile, $
                              maxseeing=maxseeing

;   ermscut = cut off frames with psfe rms gt ermscut
  
  IF n_params() EQ 0 THEN BEGIN 
      print,'-syntax corshape, run, rerun, clr, start=start, nframes=nframes, minmag=minmag, maxmag=maxmag, rcut=rcut, ermscut=ermscut, psfile=psfile'
      return
  ENDIF 
...
;  nchar=24
  ...
  FOR camcol=1,6 DO BEGIN 
      ...
      IF camcol EQ 1 THEN BEGIN 
          galflag   = 0
          starflag  = 0
          ebv       = fltarr(6L*nframes)
          ff        = fltarr(6L*nframes)
          psfmean   = fltarr(6L*nframes)
          psfsig    = fltarr(6L*nframes)
          psfe1mean = fltarr(6L*nframes)
          psfe1sig  = fltarr(6L*nframes)
          psfe2mean = fltarr(6L*nframes)
          psfe2sig  = fltarr(6L*nframes)
          foredens  = fltarr(6L*nframes)
          gl        = fltarr(6L*nframes)
          gb        = fltarr(6L*nframes)
      
          IF n_elements(psfile) EQ 0 THEN BEGIN
              psfile = fitdir+'corshape_'+rstr+'_allcols_'+colors[clr]+'_N1.ps_circ'
      ...
      ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
      ;; Read file containing psf 
      ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;      psfname, run, camcol, fname
;      psffile1 = fitdir + fname
;      print
;      print,'Reading psf file  '+psffile1
;      print
;      psfstr = mrdfits(psffile1,1)
      ...
      FOR ic = 0L, nframes-1 DO BEGIN 
          ...
          psffitname = fitdir + psffitname+'_circ'
...
;help,gal,galpsf
                  ...
                  tmp_psfe1 = (psfstr[galpsf].psfixx[clr] - $
                               psfstr[galpsf].psfiyy[clr])/psfsize
                  tmp_psfe2 = 2.*psfstr[galpsf].psfixy[clr]/psfsize
                  qu2e, tmp_psfe1, tmp_psfe2, psfe1, psfe2
                  ...
                  IF nww GT 1 THEN BEGIN 
                      ...
                      tt            = moment(psfsize)
                      psfmean[findex]   = tt[0]
                      psfsig[findex]    = sqrt(tt[1])
                      tt            = moment(pstruct[gal].reddening[clr])
                      ebv[findex]       = tt[0]
                      tt            = moment(psfe2)
                      psfe2mean[findex] = tt[0]
                      psfe2sig[findex]  = sqrt(tt[1])
                      tt            = moment(psfe1)
                      psfe1mean[findex] = tt[0]
                      psfe1sig[findex]  = sqrt(tt[1])
                      foredens[findex]  = nww
                  ENDIF ELSE BEGIN 
                      ramean        = pstruct[gal[0]].ra
                      decmean       = pstruct[gal[0]].dec
                      
                      psfmean[findex]   = psfsize[0]
                      psfsig[findex]    = 0.
                      ebv[findex]       = pstruct[gal[0]].reddening[clr]
                      psfe2mean[findex] = psfe2[0]
                      psfe2sig[findex]  = 0.
                      psfe1mean[findex] = psfe1[0]
                      psfe1sig[findex]  = 0.
                      foredens[findex]  = 1
                  ENDELSE 
                  ...
                  IF (ic MOD 20 EQ 0) OR (ic EQ 0) OR (ic EQ nframes-1) THEN BEGIN
                      ...
                      print,'Number = ',ntostr(nww),sqrt(psfmean[findex]/2.)*2.35*0.4
                  ENDIF 
                  ...
                  IF (psfe1sig[findex] le ermscut and psfe2sig[findex] le ermscut) then BEGIN
          ...
          ENDIF 
                                ;Free up some memory
          cor1=0 & obje1=0 & obje2=0 & psfe1=0 & psfe2=0 & zmag=0 & ww=0 & tmpmag=0
          psfsize=0 & psfrho4=0 & galpsf=0 & gal=0
...
;      cor1=psfsize/objsize[stars]*(4./psfrho4-1.)/(4./pstruct[stars].rho4[clr]-1.0)
          ...
          pstruct=0 & psfstr=0 & psfsize=0 & psfrho4=0 & objsize=0 & obje1=0 & obje2=0 & psfe1=0 ;Free up memory
          psfe2=0 & cor1=0 & ww=0 & stpsf=0 & stars=0 & obje1_cor2=0 & obje2_cor2=0
          findex=findex+1
...
;  print, aa, siga, bb, sigb
  ...
  plot1,xo,yo,sig,aa,bb,'!7e1 PSF '+colors[clr],'!7e1 '+colors[clr],xrange,yrange,'!7after correction'
  print,'e1PSF vs e1'
;  forprint,xo,yo,sig
  ...
  plot1,xo,yo,sig,aa,bb,'e1 PSF '+colors[clr],'e1 '+colors[clr],xrange,yrange,'before correction'

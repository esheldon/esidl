PRO make_corrected_files_match2mass, pstruct, psc_cat, xsc_cat
...
PRO make_corrected_files_read2mass, run, psc_cat, xsc_cat
...
PRO make_corrected_files_matchphotoz, pstruct, photoz, wff
...
PRO make_corrected_files_getsmear, pstruct, indices, clr, corr
...
PRO make_corrected_files_select, select_clr, pstruct, maxmag, ss, nss
...
PRO make_corrected_files, maxmag, run, rerun, camcol, $
  ...
  IF n_params() LT 4 THEN BEGIN ;Help message
      print,'-syntax make_corrected_files, maxmag, run, rerun, camcol, start=start, nframes=nframes, status=status, outdir=outdir, addphotoz=addphotoz, addbayes=addbayes, add2mass=add2mass'
      return 
  ENDIF 
  ...
  IF keyword_set(add2mass) THEN BEGIN 
      IF n_elements(psc_cat) EQ 0 AND n_elements(xsc_cat) EQ 0 THEN BEGIN 
          make_corrected_files_read2mass, run, psc_cat, xsc_cat
      ENDIF ELSE BEGIN 
  ...
  FOR ic = 0L, nfields-1 DO BEGIN 
      ...
      fxaddpar, hdr0, 'MCDATE', systime(), $
                ' Date make_corrected_files.pro was run'
      ...
      IF nps NE 0 THEN BEGIN 
          ...
          IF nwp NE 0 THEN BEGIN 
              ...
              make_corrected_files_select, 0, pstruct, maxmag, uss, unss
              IF unss NE 0 THEN flags[uss] = flags[uss] + 2b^0
                  
              make_corrected_files_select, 1, pstruct, maxmag, gss, gnss
              IF gnss NE 0 THEN flags[gss] = flags[gss] + 2b^1
                  
              make_corrected_files_select, 2, pstruct, maxmag, rss, rnss
              IF rnss NE 0 THEN flags[rss] = flags[rss] + 2b^2
                  
              make_corrected_files_select, 3, pstruct, maxmag, iss, inss
              IF inss NE 0 THEN flags[iss] = flags[iss] + 2b^3
                  
              make_corrected_files_select, 4, pstruct, maxmag, zss, znss
              ...
              IF nss NE 0 THEN BEGIN
                  ...
                  IF keyword_set(addphotoz) THEN BEGIN
                      IF rev_ind[field] NE rev_ind[field+1] THEN BEGIN 
                          ...
                          make_corrected_files_matchphotoz, pstruct, photoz, wff
                          ...
                          IF good[0] NE -1 THEN BEGIN
                              ...
                              IF ngood2 NE 0 THEN BEGIN 
                                  ...
                                  IF ngood3 NE 0 THEN BEGIN 
                                      ...
                                      make_corrected_files_getsmear,pstruct,good,clr, $
                                                              corr
                  ...
                  IF keyword_set(add2mass) THEN BEGIN 
                      make_corrected_files_match2mass, pstruct, psc_cat, xsc_cat
                  ENDIF 

...
pro corshape_allcols, run, rerun, clr, start=start, nframes=nframes,$
                      minmag=minmag, maxmag=maxmag, $
                      rcut=rcut, ermscut=ermscut, $
                      errmax=errmax, psfile=psfile, $
                      maxseeing=maxseeing, onefile=onefile, status=status
...
;   ermscut = cut off frames with psfe rms gt ermscut
  ...
  IF n_params() EQ 0 THEN BEGIN 
      print,'-syntax corshape, run, rerun, clr, start=start, nframes=nframes, minmag=minmag, maxmag=maxmag, rcut=rcut, ermscut=ermscut, psfile=psfile, onefile=onefile, status=status'
      return
  ENDIF 
  ...
  FOR camcol=1,6 DO BEGIN 
      ...
      IF camcol EQ 1 THEN BEGIN 
          ...
          IF n_elements(psfile) EQ 0 THEN BEGIN
              psfile = fitdir+'corshape_'+rstr+'_allcols_'+colors[clr]+'_N1.ps'
      ...
      FOR ic = 0L, nframes-1 DO BEGIN 
          ...
          IF NOT keyword_set(onefile) THEN BEGIN 
              ...
              IF error2 EQ 0 THEN BEGIN 
                  IF ic EQ 0 THEN BEGIN 
                      IF error2 EQ 0 THEN $
                        psfstr = mrdfits4(lun2, 1, 0, /silent)
                  ENDIF ELSE BEGIN 
                      IF error2 EQ 0 THEN $
                        psfstr = mrdfits4(lun2, 1, 0, /silent, /deja_vu )
                  ENDELSE 
              ENDIF ELSE BEGIN 
                  free_lun, lun2
                  print,!ERR_STRING
